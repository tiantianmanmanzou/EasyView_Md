...
describe('association pin merge', () => {
  it('pins a file-specific default without dropping other associations', () => {
    const { next, previous } = applyAssociationPin(
      { '*.md': 'easyviewMd.markdownEditor', '*.markdown': 'default' },
      'file:/tmp/a.md',
      'default',
    );
    expect(previous).toBeUndefined();
    expect(next).toEqual({
      '*.md': 'easyviewMd.markdownEditor',
      '*.markdown': 'default',
  ...
  it('restores the previous mapping when the pin is removed', () => {
    const pinned = applyAssociationPin(
      { '*.md': 'easyviewMd.markdownEditor', 'file:/tmp/a.md': 'easyviewMd.markdownEditor' },
      'file:/tmp/a.md',
      'default',
    );
    expect(pinned.previous).toBe('easyviewMd.markdownEditor');
    expect(removeAssociationPin(pinned.next, 'file:/tmp/a.md', pinned.previous)).toEqual({
      '*.md': 'easyviewMd.markdownEditor',
      'file:/tmp/a.md': 'easyviewMd.markdownEditor',
    });
  });

  it('deletes the pin key when it did not exist before', () => {
    const pinned = applyAssociationPin({ '*.md': 'easyviewMd.markdownEditor' }, 'file:/tmp/a.md', 'default');
    expect(removeAssociationPin(pinned.next, 'file:/tmp/a.md', pinned.previous)).toEqual({
      '*.md': 'easyviewMd.markdownEditor',
    });
  });
