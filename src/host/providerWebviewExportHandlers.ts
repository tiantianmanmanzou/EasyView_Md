...
type ExportMessage = Extract<WebviewToHostMessage, { type: 'exportXlsx' | 'exportHtml' }>;
...
export async function handleWebviewExportMessage(
  ...
  if (message.type === 'exportXlsx') {
    ...
    try {
      ...
      if (action === 'Open File') openWithDefaultApp(saveUri.fsPath);
    } catch (error: unknown) {
  ...
  try {
    if (exportImages.length === 0) {
      ...
      if (action === 'Open in Browser') openWithDefaultApp(saveUri.fsPath);
      return true;
    }
    ...
    if (action === 'Open in Browser') openWithDefaultApp(htmlPath);
  } catch (error: unknown) {
    const messageText = error instanceof Error ? error.message : String(error);
    if (messageText.includes('EBUSY') || messageText.includes('resource busy')) {
